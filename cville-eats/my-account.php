...
<div class="container">
      ...
      <tr>
        ...
        <td>
          <form action="my-account.php" method="post">
            ...
            <input type="hidden" name="review_desc_to_update" value="<?php echo $item['rest_name'] ?>" />
          </form>
        </td>
        <td>
          <form action="my-account.php" method="post">
            <input type="submit" value="Delete" name="action" class="btn btn-danger" />
            <input type="hidden" name="review_desc_to_delete" value="<?php echo $item['rest_name'] ?>" />
          </form>
        </td>
      </tr>
    <?php endforeach; ?>
  ...
  <br>
  <br>
  <h3>For Review Updates</h3>
  <h4>If you would like to submit a new review, please go to the restaurant you would like to create a review for!</h4>
  <form name="mainForm" action="my-account.php" method="post">
    <div class="mb-3">
      <label for="restaurant">Restaurant </label>
      <input type="text" name="restaurant" class="form-control" name="desc" value="<?php if ($review_to_update != null) echo $review_to_update['rest_name'] ?>" />
    </div>
    <div class="mb-3">
      <label for="comments">Comments</label>
      <input type="text" name="comments" class="form-control" value="<?php if ($review_to_update != null) echo $review_to_update['comment_desc'] ?>" />
    </div>
    <div class="mb-3">
      <label for="rating">Rating</label>
      <select name="rating" class="form-select">
        <option value=""></option>
        <option value="1" <?php if ($review_to_update != null && $review_to_update['rating'] == '1') { ?> selected <?php } ?>>1</option>
        <option value="2" <?php if ($review_to_update != null && $review_to_update['rating'] == '2') { ?> selected <?php } ?>>2</option>
        <option value="3" <?php if ($review_to_update != null && $review_to_update['rating'] == '3') { ?> selected <?php } ?>>3</option>
        <option value="4" <?php if ($review_to_update != null && $review_to_update['rating'] == '4') { ?> selected <?php } ?>>4</option>
        <option value="5" <?php if ($review_to_update != null && $review_to_update['rating'] == '5') { ?> selected <?php } ?>>5</option>
      </select>
    </div>
